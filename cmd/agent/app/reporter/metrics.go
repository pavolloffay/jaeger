...
func WrapWithMetrics(reporter Reporter, metricsNamespace string, mFactory metrics.Factory) *MetricsReporter {
	batchesMetrics := map[string]batchMetrics{}
	for _, s := range []string{zipkinBatches, jaegerBatches} {
		bm := batchMetrics{}
		metrics.Init(&bm, mFactory.Namespace(metricsNamespace, map[string]string{"format": s}), nil)
		batchesMetrics[s] = bm
	}
...
func (r *MetricsReporter) EmitZipkinBatch(spans []*zipkincore.Span) (err error) {
	return withMetrics(r.metrics[zipkinBatches], int64(len(spans)), func() error {
		return r.wrapped.EmitZipkinBatch(spans)
	})
}

// EmitBatch emits batch to collector.
func (r *MetricsReporter) EmitBatch(batch *jaeger.Batch) (err error) {
	size := int64(0)
	if batch != nil {
		size = int64(len(batch.GetSpans()))
	}
	return withMetrics(r.metrics[jaegerBatches], size, func() error {
		return r.wrapped.EmitBatch(batch)
	})
}

func withMetrics(m batchMetrics, size int64, report func() error) error {
