...
func NewCollectorProxy(builder *Builder, mFactory metrics.Factory, logger *zap.Logger) (*ProxyBuilder, error) {
	...
	b.reporter = reporter.WrapWithMetrics(r, "tchannel-reporter", mFactory)
	b.manager = httpserver.WrapWithMetrics(httpserver.NewCollectorProxy(r.CollectorServiceName(), r.Channel()), mFactory)
	return b, nil
}
